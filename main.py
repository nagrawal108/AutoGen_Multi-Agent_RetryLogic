...
# Rate limit using the rate sleep sec and if not present defaults to 1 second.
SLEEP_BETWEEN_CALLS_SEC = float(os.environ.get("RATE_SLEEP_SEC", "1.0")) 
# Chek how many times we want to retry and default is 2
...
# Helper functions to use with agent system
...
# Helpers 
# Regular expression to extract code from a Python fenced code block, with optional "python" after the opening backticks. It captures the content inside the code block for later execution or analysis.
CODE_BLOCK_RE = re.compile(r"```(?:python)?\s*(.*?)```", re.DOTALL | re.IGNORECASE) 
 
#  Extracts code from markdown agent result and format into runnable code
def extract_code_block(text: str) -> str: 
    m = CODE_BLOCK_RE.search(text or "") 
    return textwrap.dedent(m.group(1).strip()) if m else textwrap.dedent(text or "").strip() 
 
# This acts like a tool to execute python code extracted from the test python file.
def run_py_tests(solution_code: str, test_code: str, timeout_sec: int = 10) -> Tuple[bool, str]: 
    with tempfile.TemporaryDirectory() as tmp: 
        ...
        try: 
            # Executes a subprocess to run the test file
...
# Runs an async call to the agent and logs the response. It sends a message to the agent,
#  waits for the response, and then extracts the content of the response.
#  The function also logs the interaction with a timestamp, agent name, label, and content
#  for later analysis. The sleep at the beginning helps to manage rate limits when making calls to the LLM.
...
# Main entry point for prompting the agent
def ask(agent: AssistantAgent, message: str, label: str) -> str: 
    return asyncio.run(_ask_async(agent, message, label))

# Create the agents with specific system messages that define their roles and instructions. 
# Each agent is designed to perform a specific task in the process of generating, testing, and 
# debugging code for checking if a string is a palindrome.

# This is an autogen assistant agent that generates Python code for a function to check if a string is a palindrome.
...
async def main(): 
    ...
    # 1) CodeGen creates solution.py  (manual sequential orchestration for clarity of the demo)
    ...
    # Basic error handling to confirm we are getting the right function of the output
    # In reality we could loop this to ensure we get the right code.
